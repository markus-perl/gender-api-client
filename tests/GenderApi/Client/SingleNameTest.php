...
class SingleNameTest extends TestCase
{
    ...
    public function testServerNotReachable()
    {
        ...
        if ($this->doMock) {
            $downloader = $this->createMock(FileGetContents::class);
            $downloader->method('download')
                ->willThrowException(new \GenderApi\Client\Downloader\NetworkErrorException('file_get_contents(https://gender-api.com/wrong-url?key=QRFtEEezuBSYLJFYSJ&name=markus): failed to open stream: HTTP request failed! HTTP/1.1 404 Not Found'));
            $genderApiClient->setDownloader($downloader);
        }
